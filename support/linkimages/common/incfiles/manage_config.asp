...
Function get_linkimages_url(ByVal strers)
  If left(strers, "4") = "http" Then
    get_linkimages_url = strers
  Else
    get_linkimages_url = get_nurlpath() & "/" & strers
  End If
End Function
...
Sub jtbc_cms_admin_manage_adddisp()
  ...
  Dim tmpimage: tmpimage = left_intercept(get_str(request.Form("image")), 200)
  rs.addnew
...
Sub jtbc_cms_admin_manage_createjsdisp()
  ...
  Set rs = conn.Execute(sqlstr)
  Dim tmpstr, tmpastr, tmprstr, tmptstr
  tmpstr = itake("linkimages." & tjstpl, "tpl")
