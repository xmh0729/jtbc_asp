...
Sub jtbc_cms_admin_manage_adddisp()
  ...
  rs(cfname("url")) = left_intercept(get_str(request.Form("url")), 255)
  rs(cfname("encode")) = left_intercept(get_str(request.Form("encode")), 50)
  rs(cfname("retimevalue")) = get_num(request.Form("retimevalue"), 0)
  rs(cfname("retimetype")) = get_num(request.Form("retimetype"), 0)
...
Sub jtbc_cms_admin_manage_editdisp()
  ...
  If Not rs.EOF Then
    ...
    rs(cfname("retimevalue")) = get_num(request.Form("retimevalue"), 0)
    rs(cfname("retimetype")) = get_num(request.Form("retimetype"), 0)
    rs(cfname("retime")) = get_date(request.Form("retime"))
    rs(cfname("time")) = get_date(request.Form("time"))
    rs.Update
