...
ncontrol = "select,hidden,good,top,delete"
...
Sub jtbc_cms_admin_manage_list()
  ...
  sqlstr = "select top " & nlisttopx & " * from " & ndatabase & "," & sort_database & " where " & ndatabase & "." & cfname("class") & "=" & sort_database & "." & sort_idfield & " and " & sort_database & "." & cfnames(sort_fpre, "lng") & "='" & slng & "' and " & sort_database & "." & cfnames(sort_fpre, "genre") & "='" & ngenre & "'"
  ...
  If search_field = "top" Then sqlstr = sqlstr & " and " & ndatabase & "." & cfname("top") & "=" & get_num(search_keyword, 0)
  ...
  For jcuti = 1 To npagesize
    If Not rs.EOF Then
      ...
      tmptstr = Replace(tmptstr, "{$classid}", get_num(rs(sort_idfield), 0))
...
Sub jtbc_cms_admin_manage_adddisp()
  ...
  rs(cfname("topic_b")) = get_num(request.Form("topic_b"), 0)
  rs(cfname("topic_color")) = left_intercept(get_str(request.Form("topic_color")), 50)
  ...
  rs(cfname("top")) = get_num(request.Form("top"), 0)
  rs.Update
...
Sub jtbc_cms_admin_manage_editdisp()
  Dim tid, tmpimage, tcontent_images_list, tbackurl
  Dim tmpclass: tmpclass = get_num(request.Form("sort"), 0)
  If tmpclass = 0 Then Call client_alert(Replace(itake("global.lng_public.insert_empty", "lng"), "[]", "[" & itake("global.lng_config.sort", "lng") & "]"), -1)
  tid = get_num(request.querystring("id"), 0)
  tmpimage = left_intercept(get_str(request.Form("image")), 250)
  tcontent_images_list = left_intercept(get_str(request.Form("content_images_list")), 10000)
